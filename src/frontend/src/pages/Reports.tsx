...
function IssueVsPackingSummaryReport({
  ...
  const oeRealisation =
    totalOeIssued > 0 ? (totalOePacked / totalOeIssued) * 100 : 0;
  const ringRealisation =
    totalRingIssued > 0 ? (totalRingPacked / totalRingIssued) * 100 : 0;
  const overallRealisation =
    totalIssued > 0 ? (totalPacked / totalIssued) * 100 : 0;
    ...
    <div className="space-y-6" data-ocid="issue_packing_summary.section">
      ...
      <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-5">
          ...
          <div className="rounded-lg border bg-emerald-50/60 px-5 py-4 flex flex-col gap-1">
      ...
      <div className="bg-white border rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b bg-slate-50/60 flex items-center gap-2">
          ...
          <h3 className="font-semibold text-sm text-slate-800">
            Balance & Net Realisation Summary
          </h3>
        </div>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            ...
            >
              ...
              <TableCell
                className={`text-xs text-right tabular-nums font-semibold ${oeRealisation >= 90 ? "text-green-600" : oeRealisation >= 70 ? "text-amber-600" : "text-red-600"}`}
              >
                {totalOeIssued > 0 ? `${oeRealisation.toFixed(2)}%` : "—"}
              </TableCell>
            </TableRow>
            <TableRow className="hover:bg-slate-50/40">
              ...
              <TableCell
                className={`text-xs text-right tabular-nums font-semibold ${ringRealisation >= 90 ? "text-green-600" : ringRealisation >= 70 ? "text-amber-600" : "text-red-600"}`}
              >
                {totalRingIssued > 0 ? `${ringRealisation.toFixed(2)}%` : "—"}
              </TableCell>
            </TableRow>
          </TableBody>
          <tfoot>
            <tr className="border-t-2 border-slate-300 bg-slate-100/70">
              ...
              <td
                className={`px-4 py-2 text-xs font-bold text-right tabular-nums ${overallRealisation >= 90 ? "text-green-600" : overallRealisation >= 70 ? "text-amber-600" : "text-red-600"}`}
              >
                {totalIssued > 0 ? `${overallRealisation.toFixed(2)}%` : "—"}
              </td>
            </tr>
          </tfoot>
        </Table>
